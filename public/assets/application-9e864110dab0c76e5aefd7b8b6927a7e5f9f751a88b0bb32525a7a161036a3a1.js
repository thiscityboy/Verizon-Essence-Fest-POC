...
"opacity"){el.style.opacity=progress[i];return}if(isTransformFunction(prop)){if(!transforms){transforms=new Map;transforms.set("functions",[]);transforms.set("values",[])}transforms.get("functions").push(prop);transforms.get("values").push(progress[i]);return}el.setAttribute(prop,progress[i])});if(!transforms)return;if(!transform)transform="transform"in document.body.style?"transform":"-webkit-transform";el.style[transform]=combineTransformFunctions.apply(undefined,_toConsumableArray(transforms.values()))}}();
